...
[covariance] = load('phantom_32_200nam_20240708_raw_cov.mat');
...
%% read full data
[raw] = fiff_setup_read_raw(infile,1); %maxshield=1 keeps it unprocessed, with IAS off it doesnt matter
% %with IAS on. Default is on
[data,times] = fiff_read_raw_segment(raw);
% raw_mat = load("phantom_1000nam_default_IASoff_data.mat");
% data = raw_mat.raw_data; times = raw_mat.raw_times;

%% Deal with Bad channels
% we only want channels beginning with 'MEG...'
bad_chans = []; good_names={};
k=1;
j=1;
for i = 1:nchan
    if contains(info.ch_names{1,i},'MEG') 
        good_names{1,j}=info.ch_names{1,i};
        j=j+1;
    else
        bad_chans(k) = i;
        bad_names{1,k}=info.ch_names{1,i};
        k=k+1;
    end
end 
...
EY(:,bad_chans)=[];
EZ(:,bad_chans)=[];
%[R,EX,EY,EZ] = fiff_getpos(infile, coordsys);

%visualize sensor array
figure(1)
hold on
...
% %%%% check the subspace angle from a good time point between the data and
% %%%% SSS matrix to see how bad is. Noise covariance of sensors, does not
% %%%% contain the environment
% % for i=(1:size(times,2))
% %     check_data_sss(i) = subspace(phi_0(:,i), S)*180/pi;
% % end
% % check_data_s_dmin = min(check_data_sss);
% % check_data_s_dmax = max(check_data_sss);
% % check_data_s_dav = mean(check_data_sss);
% 
% 
% %% save files
...
% save("phantom_32_200nam_20240708_mat_mag_dev_sss.mat", 'data_rec_vsh','-v7.3')
% save("phantom_32_200nam_20240708_mat_mag_dev_fos.mat", 'data_rec_fos','-v7.3')
